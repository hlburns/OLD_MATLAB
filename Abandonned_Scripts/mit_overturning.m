...
% vmask = NaN mask I guess 1s and NaNs from HFaCS file
...
% $Name:  $
  ...
  if nargin < 5 % nargin = number of input arguments into function
    addlayer = 0;
  elseif (nargin > 5 && nargin < 4)
    error('needs 4 or 5 arguments')
  end
  [nx, ny, nz] = size(vmask); %vmask in xyz
  %for each vertical layer work out DV 
  DV=zeros(nx,ny,nz);
  for kz=1:nz 
      DV(:,:,kz) = (vmask(:,:,kz).*dx)*dz(kz);
  end

  % mask for stream function zonal average and call change function change
  % values that do not = NaN to 1 in y-z. 
  ...
  if iscell(v)
    ...
    psi=zeros(ny,nz,nt);
    for k=1:nt
      vdxdz = squeeze(v(:,:,:,k)).*DV; %(dxdzs.*vmask);
